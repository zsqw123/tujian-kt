...
import io.nichijou.viewer.ext.forEach
...
internal abstract class RecyclingPagerAdapter<VH : RecyclingPagerAdapter.ViewHolder> : PagerAdapter() {
  ...
  override fun restoreState(state: Parcelable?, loader: ClassLoader?) {
    if (state != null && state is Bundle) {
      ...
      savedStates = if (state.containsKey(STATE)) state.getSparseParcelableArray(STATE)
        ?: SparseArray() else SparseArray()
    }
  ...
  private class RecycleCache constructor(private val adapter: RecyclingPagerAdapter<*>) {
