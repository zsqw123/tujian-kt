...
class Bubble {
  ...
  fun draw(canvas: Canvas, paint: Paint, size: Float, t: Long) {
    ...
    for (i in 0..7) {
      ...
      iX = (cos(iTheta.toDouble()) * size.toDouble() * WOBBLE_AMOUNT.toDouble()).toFloat()
      iY = (sin(iTheta.toDouble()) * size.toDouble() * WOBBLE_AMOUNT.toDouble()).toFloat()
      jX = (cos(jTheta.toDouble()) * size.toDouble() * WOBBLE_AMOUNT.toDouble()).toFloat()
      jY = (sin(jTheta.toDouble()) * size.toDouble() * WOBBLE_AMOUNT.toDouble()).toFloat()
      if (i == 0) {
  ...
  companion object {
    ...
    private const val SPLINE_8 = 0.2652031f
    private val SQRT_HALF = sqrt(0.5).toFloat()
    ...
    init {
      ...
      hsv[2] = 1f
      for (i in 0..7) {
        CIRCLE_8_AX[i] = (CIRCLE_8_X[i] - cos(CIRCLE_8_THETA[i].toDouble()) * SPLINE_8).toFloat()
        CIRCLE_8_BX[i] = (CIRCLE_8_X[i] + cos(CIRCLE_8_THETA[i].toDouble()) * SPLINE_8).toFloat()
        CIRCLE_8_AY[i] = (CIRCLE_8_Y[i] - sin(CIRCLE_8_THETA[i].toDouble()) * SPLINE_8).toFloat()
        CIRCLE_8_BY[i] = (CIRCLE_8_Y[i] + sin(CIRCLE_8_THETA[i].toDouble()) * SPLINE_8).toFloat()
        hsv[0] = i * 360 / 8f
        TEST_COLORS[i] = -0x7f000001 and Color.HSVToColor(hsv)
      }
    }

    private const val MIN_PERIOD = 1500
    private const val MAX_PERIOD = 2500
